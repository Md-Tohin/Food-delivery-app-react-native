...
const SignupScreen = ({ navigation }) => {
  ...
  const [emailErorMessage, setEmailErrorMessage] = useState('');
  const [usernameErorMessage, setUsernameErrorMessage] = useState('');
  ...
  const checkUserExist = async (type, value) => {
    if (value?.length > 0) {
      AuthenticationService.checkUserExist(type, value).then((response) => {
        if (response?.status) {
          type === 'email' && emailErorMessage ? setEmailErrorMessage('') : null;
          type === 'username' && usernameErorMessage ? setUsernameErrorMessage('') : null;
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.content}>
        Enter your Name, Email and Password for sign up, Or {''}
        <Text onPress={() => navigation.navigate('registerPhone')} style={styles.subcontentText}>
      ...
      {usernameErorMessage ? (
        <Text style={styles.errorMessage}>{usernameErorMessage}</Text>
      ) : (
        <Separator height={15} />
      )}
      ...
      {emailErorMessage ? (
        <Text style={styles.errorMessage}>{emailErorMessage}</Text>
      ) : (
        <Separator height={15} />
      )}
